...
export default {
  ...
  data() {
    ...
    return {
      ...
      meanList:[140, 232, 101, 264, 90, 340, 280],//维护均值数据
      varianceList:[120, 132, 101, 134, 90, 230, 210],//维护方差数据
      modeList:[120, 282, 111, 234, 220, 340, 310],//维护众数数据
      medianList:[],//中位数
      abnormalList:[],//异常值
      dynamic_bar_data:[90,12,10,80,34,67,65,44],//动态柱状图数据
    }
  },
  methods: {
    //五档健康状态折线图数据获取事件函数
    fetchDataAndCreateChartGraphLine: function () {
      ...
      // axios请求后台API
      _this.$axios.get('http://127.0.0.1:5000/api/graphLine_random')
    ...
    //饼状图数据获取事件函数
    fetchDataAndCreateChartGraphPie: function () {
      // 饼状图美化的一些东西
      const piePatternSrc =
      ...
      var _this = this; // 保证在回调中this指向Vue实例
      // axios请求后台API
      _this.$axios.get('http://127.0.0.1:5000/api/graphPie_top')
        .then(function (response) {
          // response.data 是您从后台得到的数据
          var data = response.data;
          ...
          // 设置echarts图表配置
          _this.graph_pie.setOption(optino_pie);
        })
        .catch(function (error) {
          // 处理请求错误
          console.log(error);
    ...
    //堆叠折线图数据获取事件函数
    fetchDataAndCreateChartGraphStack: function (){
      var _this = this; // 保证在回调中this指向Vue实例
      // axios请求后台API
      _this.$axios.get('http://127.0.0.1:5000/api/graph_stack')
        .then(function (response) {
          // response.data 是您从后台得到的数据
          var data = response.data;
          ...
          // 使用点操作符获取对象属性
          _this.meanList = data.mean;
          _this.medianList = data.median;
          _this.modeList = data.mode;
          _this.varianceList = data.variance;
          _this.abnormalList = data.abnormal;

          console.log('平均值列表:',_this.meanList);
          console.log('中位数列表:', _this.medianList);
          console.log('众数列表:', _this.modeList);
          console.log('方差列表:', _this.varianceList);
          console.log('异常值列表:', _this.abnormalList);
          ...
          var option_stack = {
            ...
            // 图例
            //均值 series not exists. Legend data should be same with series name or data name。也就是图例得和下面series中的name相同
            legend: {
              data: ['电压均值', '电压众值', '电压中位数', '电压方差', '异常值']
            },
            toolbox: {
              feature: {
                saveAsImage: {}
            ...
            series: [
              ...
              },
              {
                ...
                data: _this.abnormalList
              }
            ]
          };

          // 设置echarts图表配置
          _this.graph_stack.setOption(option_stack);
        })
        .catch(function (error) {
          // 处理请求错误
          console.log(error);
  ...
  mounted() {
    ...
    //饼状图的东西
     _this.graph_pie = echarts.init(document.getElementById('graph_pie'));
    //初始化请求数据，并创建饼状图图表
     _this.fetchDataAndCreateChartGraphPie();

    //渐变堆叠面积图
    _this.graph_stack  = echarts.init(document.getElementById('graph_stack'));
    _this.fetchDataAndCreateChartGraphStack();


    //动态柱状图的东西 —— 这里是拿的官网的东西，得逐步写成上面这种发送请求的
    var chartBar = echarts.init(document.getElementById('graph_bar'));
    const data = [];
    for (let i = 0; i < 8; ++i) {
      // data.push(Math.round(Math.random() * 200));
      data.push(Math.round(this.dynamic_bar_data[i]));
    }
    // data : this.dynamic_bar_data;
    var option_bar = {
      xAxis: {
        max: 'dataMax'
      },
    ...
    chartGauge.setOption(option_gauge);
